...
(function () {
  ...
  const MOB_TYPES = {
    ...
    crypt_ghoul: { name: '크립트 구울', kind: 'humanoid', color: 0x5a8a5a, hp: 2000, dmg: 31, fixedStats: true,   /* V96: 크립트 고정몹 — 위키 실HP 유지 */ gear: { sword: 0xf7d84a }, xp: 25, coins: 15, speed: 2.2, books: ['giant_killer', 'execute'], drops: [{ key: 'rotten_flesh', n: 1 }, { key: 'rotten_flesh', n: 1, chance: 0.5 }], tierCap: 3 },   /* V103: 위키 실측 — 썩은 살점 1-2만(골드 미드롭, 골드는 골든 구울) */
    golden_ghoul: { name: '골든 구울', kind: 'humanoid', color: 0xd8b23a, hp: 4500, dmg: 45, fixedStats: true,   /* V96: 크립트 고정몹 */ gear: { helmet: 0xf7d84a, sword: 0xf7d84a }, xp: 45, coins: 60, speed: 2.3, books: ['looting'], drops: [{ key: 'rotten_flesh', n: 2 }, { key: 'gold', n: 1 }, { key: 'gold', n: 1, chance: 0.5 }, { key: 'gold', n: 1, chance: 0.5 }, { key: 'talisman_wealth_rune', n: 1, chance: 0.02 }], tierCap: 4 },   /* V104: 위키 실측 — 썩은 살점 1-2 + 골드 잉곳 1-10 */
    ...
    wolf: { name: '늑대', kind: 'quad', color: 0x9a9a9a, hp: 160, dmg: 20, xp: 10, coins: 4, speed: 2.8, books: ['first_strike', 'looting'], drops: [{ key: 'bone', n: 1 }], tierCap: 2 },   // V78: 위키 — 뼈 1개
    old_wolf: { name: '올드 울프', kind: 'quad', color: 0x5a5a62, hp: 900, dmg: 80, xp: 60, coins: 25, speed: 3.0, scale: 1.4, books: ['first_strike', 'experience'], drops: [{ key: 'bone', n: 1 }], tierCap: 4 },
    ...
    miner_zombie: { name: '광부 좀비', kind: 'humanoid', color: 0x7a6a4a, hp: 200, dmg: 28, fixedStats: true,   /* V96: 골드광산 명명 좀비 고정몹 */ gear: { helmet: 0xd8d8d4, tool: 0x9c7a44 }, xp: 15, coins: 5, speed: 1.8, books: ['efficiency'], drops: [{ key: 'rotten_flesh', n: 1 }], tierCap: 2 },
    lapis_zombie: { name: '청금석 좀비', kind: 'humanoid', color: 0x2a4fc0, hp: 260, dmg: 32, fixedStats: true,   /* V96: 딥캐번 명명 좀비 고정몹 */ gear: { helmet: 0x1f4fc0, chest: 0x1f4fc0 }, xp: 18, coins: 6, speed: 1.8, books: ['fortune'], drops: [{ key: 'rotten_flesh', n: 1 }, { key: 'rotten_flesh', n: 1, chance: 0.5 }], tierCap: 3 },   /* V103: 위키 실측 — 청금석(lapis)은 광석 채굴에서만, 라피스 좀비는 썩은 살점 1-2 + 라피스 갑옷 1%/부위(mobDropTable 오버라이드) */
    ...
    redstone_pigman: { name: '레드스톤 피그맨', kind: 'humanoid', color: 0xc86a6a, hp: 400, dmg: 42, fixedStats: true,   /* V96: 딥캐번 레드스톤 고정몹 */ xp: 24, coins: 8, speed: 2.0, books: ['fortune', 'efficiency'], drops: [{ key: 'gold_nugget', n: 2 }], tierCap: 3 },   /* V103: 위키 실측 — 레드스톤 0-5 @50%, 골드 너겟 0-2 @50%(고정 3개 아님) */
    ...
    broodmother: { name: '브루드마더', kind: 'spider', color: 0x4a2050, hp: 6000, dmg: 90, xp: 150, coins: 80, speed: 2.2, scale: 2.4, books: ['bane_of_arthropods', 'rejuvenate'], drops: [{ key: 'string', n: 1 }, { key: 'spider_eye', n: 1 }], tierCap: 5 },
    ...
    blaze: { name: '블레이즈', kind: 'blaze', color: 0xe8a020, hp: 600, dmg: 50, xp: 35, coins: 8, speed: 2.0, books: ['fire_aspect', 'thunderlord'], drops: [{ key: 'blaze_rod', n: 1 }, { key: 'blaze_rod', n: 1, chance: 0.5 }], tierCap: 4 },   /* V104: 위키 실측 — 블레이즈 막대 1-2(100%/50%) */   // V79: 위키 — 블레이즈 막대 100%
    wither_skeleton: { name: '위더 스켈레톤', kind: 'tall', color: 0x2a2a2a, hp: 900, dmg: 65, xp: 45, coins: 10, speed: 2.2, books: ['titan_killer', 'dragon_hunter'], drops: [{ key: 'bone', n: 3 }, { key: 'coal', n: 1, chance: 0.5 }, { key: 'enchanted_coal', n: 1, chance: 0.01 }], tierCap: 4 },   /* V104: 위키 실측 — 뼈 100% + 석탄 50% + 인챈티드 석탄 1% */
    magma_cube: { name: '마그마 큐브', kind: 'slime', color: 0xd2541f, hp: 350, dmg: 35, xp: 22, coins: 6, speed: 1.5, books: ['hardened', 'thorns'], drops: [{ key: 'magma_cream', n: 1 }, { key: 'magma_cream', n: 1, chance: 0.5 }, { key: 'magma_cream', n: 1, chance: 0.5 }], tierCap: 3 },   /* V104: 위키 실측 — 마그마 크림 1-3(100%) */   // V79: 위키 — 마그마 크림 100%(컬렉션 공급원)
    pigman: { name: '피그맨', kind: 'humanoid', color: 0xe6a8ad, hp: 450, dmg: 48, xp: 28, coins: 8, speed: 2.1, books: ['vitality'], drops: [{ key: 'gold_nugget', n: 2 }], tierCap: 3 },   /* V104: 위키 실측 — 골드 너겟 2(100%) */
    enderman: { name: '엔더맨', kind: 'tall', color: 0x1a1a22, hp: 800, dmg: 60, hpAnchors: [[42, 4500], [45, 6000], [50, 9000]], fixedStats: true, xp: 40, coins: 8, speed: 2.6, books: ['ender_slayer', 'sugar_rush'], drops: [{ key: 'ender_pearl', n: 1 }, { key: 'ender_pearl', n: 1, chance: 0.5 }, { key: 'ender_pearl', n: 1, chance: 0.5 }, { key: 'enchanted_ender_pearl', n: 1, chance: 0.01 }], tierCap: 4 },   /* V104: 위키 실측 — 엔더 진주 1-3(100%) + 인챈티드 엔더 진주 1% */   // V81 HP앵커 유지 + V97(C11): 데미지 앵커(500~700, 동일구역 젤롯63·워처72의 8배로 즉사) 제거 → fixedStats로 고정 dmg 60(HP는 hpAnchors가 먼저 평가되어 탱키 유지, dmg만 피어 정렬)
    endermite: { name: '엔더마이트', kind: 'quad', color: 0x5a3a6a, hp: 320, dmg: 66, fixedStats: true,   /* V96: 엔드 고정몹(공격력 폭주 수정) */ xp: 20, coins: 5, speed: 3.2, scale: 0.5, books: [], drops: [{ key: 'end_stone', n: 1 }, { key: 'end_stone', n: 1, chance: 0.5 }, { key: 'ender_shard', n: 1, chance: 0.1 }], tierCap: 3 },   /* V104: 위키 실측 — 엔드스톤 1-2(엔더 파편은 게임 자원 경제상 소량 유지) */
    ...
    obsidian_defender: { name: '흑요석 수호자', kind: 'tall', color: 0x2a2040, hp: 500, dmg: 29, fixedStats: true,   /* V96: 엔드 Lv55 고정몹(워처 쌍) */ xp: 30, coins: 8, speed: 1.8, books: ['protection', 'hardened'], drops: [{ key: 'obsidian', n: 1 }, { key: 'obsidian', n: 1, chance: 0.5 }, { key: 'enchanted_obsidian', n: 1, chance: 0.01 }], tierCap: 5 },   /* V104: 위키 실측 — 흑요석 다수 + 인챈티드 흑요석 1% */
    watcher: { name: '워처', kind: 'tall', color: 0x3a2a52, hp: 480, dmg: 72, fixedStats: true,   /* V96: 엔드 Lv55 고정몹(공격력 폭주 수정) */ xp: 32, coins: 8, speed: 2.4, books: ['venomous'], drops: [{ key: 'ender_pearl', n: 1 }, { key: 'ender_pearl', n: 1, chance: 0.5 }, { key: 'enchanted_bone', n: 1, chance: 0.01 }, { key: 'ender_shard', n: 1 }], tierCap: 5 },
    // V97(C15): standalone ender_dragon 제거 — DRAGON_TYPES(8종)로 스폰되며 이 키는 어디서도 참조되지 않던 죽은 정의였음
    sea_walker: { name: '바다 보행자', kind: 'humanoid', color: 0x2a6a8a, hp: 300, dmg: 25, xp: 20, coins: 6, speed: 1.6, books: ['vampirism', 'protection'], drops: [{ key: 'rotten_flesh', n: 1 }, { key: 'rotten_flesh', n: 1, chance: 0.5 }, { key: 'rotten_flesh', n: 1, chance: 0.25 }], tierCap: 3 },
  ...
  MOB_TYPES.ghast = { name: '가스트', kind: 'ghast', color: 0xe8e8e8, hp: 1200, dmg: 90, xp: 55, coins: 18, speed: 1.6, fly: true, drops: [{ key: 'ghast_tear', n: 1 }, { key: 'ghast_tear', n: 1, chance: 0.5 }, { key: 'gunpowder', n: 2 }, { key: 'enchant_book_dragon_hunter', n: 1, chance: 1 / 600 }], tierCap: 4 };   // V79: 위키 — 가스트 눈물 100%(+50% 2번째)
  ...
  SPAWN_AREAS = SPAWN_AREAS.concat([
    ...
    // 파크/버섯 사막
    { world: 'park', x: 72, z: 82, r: 34, rMin: 0, types: ['wolf'], lv: [8, 15], cap: 4, respawn: 15 },
