/* =========================================================================
   ...
   ⚠ 베타: 카드 비용은 표준 대칭 패턴(원작 구조 동일). 리서치 검증표로 교체 예정.
...
/* ---- 90장 카드 생성(보너스색마다 표준 패턴을 다른 4색에 회전 배치 = 스플랜더 대칭 구조) ---- */
const SP_T1 = [ {p:0,a:[1,1,1,1]},{p:0,a:[2,1,1,1]},{p:0,a:[2,2,1,0]},{p:0,a:[3,1,1,0]},{p:0,a:[2,1,0,0]},{p:0,a:[2,0,2,0]},{p:0,a:[3,0,0,0]},{p:1,a:[4,0,0,0]} ];
const SP_T2 = [ {p:1,a:[3,2,2,0]},{p:1,a:[0,2,3,3]},{p:2,a:[1,4,2,0]},{p:2,a:[5,3,0,0]},{p:2,a:[5,0,0,0]},{p:3,a:[6,0,0,0]} ];
const SP_T3 = [ {p:3,a:[3,3,5,3]},{p:4,a:[0,7,0,0]},{p:4,a:[3,6,3,0]},{p:5,a:[7,3,0,0]} ];
const SP_CARDS = (() => {
  const out = []; let id = 0;
  [[1,SP_T1],[2,SP_T2],[3,SP_T3]].forEach(([t,pats]) => {
    for (let b = 0; b < 5; b++) for (const pat of pats) {
      const c = [0,0,0,0,0]; const others = [0,1,2,3,4].filter(x => x !== b);
      pat.a.forEach((amt,i) => { if (amt > 0) c[others[i]] = amt; });
      out.push({ id: id++, t, b, p: pat.p, c });
    }
  });
  return out;
})();
